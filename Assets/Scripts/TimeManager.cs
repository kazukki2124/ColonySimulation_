...
public class TimeManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            SetTimeScale(0f);   //PauseButton‚ª‰Ÿ‚³‚ê‚½‚ÉSetTimeScale‚ğ0‚É‚µ‚Ä’â~‚·‚é
            SEManager.Instance.PlaySE(StopSE);
        });

        PlayButton.onClick.AddListener(() =>
        {
            SetTimeScale(1f);    //PlayButton‚ª‰Ÿ‚³‚ê‚½‚ÉSetTimeScale‚ğ1‚É‚µ‚Ä“™”{‘¬‚É‚·‚é
            SEManager.Instance.PlaySE(PlaySE);
        });

        Speed2xButton.onClick.AddListener(() =>
        {
            SetTimeScale(2f); //Speed2xButton‚ª‰Ÿ‚³‚ê‚½‚ÉSetTimeScale‚ğ2‚É‚µ‚Ä2”{‘¬‚É‚·‚é
            SEManager.Instance.PlaySE(SpeedUpSE);
        });

        Speed3xButton.onClick.AddListener(() =>
        {
            SetTimeScale(3f); //Speed3xButton‚ª‰Ÿ‚³‚ê‚½‚ÉSetTimeScale‚ğ3‚É‚µ‚Ä3”{‘¬‚É‚·‚é
            SEManager.Instance.PlaySE(SpeedUpSE);
        });
    }
